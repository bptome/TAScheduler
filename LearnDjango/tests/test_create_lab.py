...
class TestAdminCreateLab(TestCase):
    ...
    def test_valid_entry_lab(self):
        self.test_admin.create_lab("My Lab", "My lab for course 1", self.course1)
        new_lab = ["My Lab", "My lab for course 1"]
    ...
    def test_valid_two_lab_course(self):
        self.test_admin.create_lab("My Lab", "My lab for course 1", self.course1)
        self.test_admin.create_lab("My Lab 2", "My second Lab for Course 1", self.course1)
        labs = []
        for junction in LabCourseJunction.objects.filter(course_id=self.course1):
            labs.append(model_lab_to_test_array(junction.lab_id))
        new_lab = ["My Lab", "My lab for course 1"]
        new_lab2 = ["My Lab 2", "My second Lab for Course 1"]
    ...
    def test_valid_lab_two_courses_in_system(self):
        get_course_one_lab()
        self.test_admin.create_lab("My Lab", "My lab for course 2", self.course1)
        new_lab = ["My Lab", "My lab for course 2"]
        labs = []
        for junction in LabCourseJunction.objects.filter(course_id=self.course1):
            labs.append(model_lab_to_test_array(junction.lab_id))
        failure_msg = "Lab should be assigned to the course"
    ...
    def test_taken_lab_id_message(self):
        self.test_admin.create_lab("My Lab", "My Lab for Course 2", self.course1)
        message = self.test_admin.create_lab("My Lab", "My Lab for Course 2", self.course1)
...
class TestOtherUserCreateLab(TestCase):
    ...
    def test_instructor_attempt_message(self):
        ...
        message = test_instructor.create_lab("My Lab", "My Lab for Course 2", self.course1)
        failure_msg = "Correct error message not returned for instructor trying to create a lab"
        self.assertEqual(message, self.error_message, msg=failure_msg)

    def test_TA_attempt_message(self):
        test_TA = TA(1, "", "", "", "", "")
        message = test_TA.create_lab("My Lab", "My Lab for Course 2", self.course1)
        failure_msg = "Correct error message not returned for TA trying to create a lab"
        self.assertEqual(message, self.error_message, msg=failure_msg)

    def test_instructor_attempt_db_changes(self):
        test_instructor = Instructor(1, "", "", "", "", "")
        test_instructor.create_lab("My Lab", "My Lab for Course 2", self.course1)
    ...
    def test_TA_attempt_db_changes(self):
        test_TA = TA(1, "", "", "", "", "")
        test_TA.create_lab("My Lab", "My Lab for Course 2", self.course1)
