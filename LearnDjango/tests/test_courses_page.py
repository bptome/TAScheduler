...
from .TestAdminDataAccess import add_user_to_test_database, add_second_admin, add_instructor, add_ta
...
class InstructorAccess(TestCase):
    def setUp(self):
        self.user_a_info = ["Jane Doe", "apple", "doe@uwm.edu", "Random ave", 2, "(123)143-4867"]
        self.user_a = User(99, "Jane Doe", "apple", "doe@uwm.edu", "Random ave", 2, "(123)143-4867").save()
        self.client = Client()
        self.firstResponse = self.client.post("/", {"email": "doe@uwm.edu", "password": "apple"}, follow=True)

    def test_no_other_users(self):
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Only logged in Instructor should be visible"
        self.assertEqual(response.context['people'], [self.user_a_info], msg=failure_msg)

    def test_admin(self):
        test_array = [self.user_a_info, add_second_admin()]
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Instructor and Admin should be visible"
        self.assertEqual(response.context['people'], test_array, msg=failure_msg)

    def test_instructor(self):
        test_array = [self.user_a_info, add_instructor()]
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Two instructors should be visible"
        self.assertEqual(response.context['people'], test_array, msg=failure_msg)

    def test_TA(self):
        test_array = [self.user_a_info, add_ta()]
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Instructor and TA should be visible"
        self.assertEqual(response.context['people'], test_array, msg=failure_msg)

    def test_three_different_types(self):
        test_array = [self.user_a_info, add_second_admin(), add_instructor(), add_ta()]
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Two instructors, admin and TA should be visible"
        self.assertEqual(response.context['people'], test_array, msg=failure_msg)
    ...
    def test_name(self):
        response = self.client.get("/people/", {}, follow=True)
        failure_msg = "Name should match Instructor's name"
        self.assertEqual(response.context['name'], self.user_a.name, msg=failure_msg)
